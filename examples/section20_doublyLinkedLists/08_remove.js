/**
    ...
    ●  If the index is the same is equal length -1, POP
    ● Use the GET method to retrieve the item to be removed
    ● Update the next and prev properties to remove the found
        node from the list
    ● Set the next and prev null on the found node
...
class DoublyLinkedList {
  ...
  remove(index) {
    // Check if the index is out of bounds (less than 0 or greater/equal to the length)
    if (index < 0 || index >= this.length) {
      // Return undefined if the index is out of bounds
      return undefined;
    }
    // Remove the node at the beginning of the list (index = 0)
    if (index === 0) {
      // Call the shift() method to remove and return the first node
      return this.shift();
    }
    // Remove the node at the end of the list (index = length - 1)
    if (index === this.length - 1) {
      // Call the pop() method to remove and return the last node
      return this.pop();
    }

    // Retrieve the node before the removed node
    let prevNode = this.get(index - 1);
    // Retrieve the node to be removed at the specified index
    let removed = prevNode.next;
    // Retrieve the node after the removed node
    let afterNode = removed.next;

    // Update the pointers of the adjacent nodes to remove the found node from the list
    ...
    // Decrement the length of the list to reflect the removal of the
    this.length--;
    // Return the removed node
    return removed;
  }
